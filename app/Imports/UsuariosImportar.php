...
class UsuariosImportar implements ToModel, WithHeadingRow, SkipsEmptyRows
{
    public function model(array $row) {

        $data_nascimento = null;

        if (empty($row["nome_completo"]) && empty($row["usuario"]) && empty($row["email"]) && empty($row["senha"]) && empty($row["cpf"]) && empty($row["data_nascimento"]) && empty($row["celular"]) && empty($row["genero"])) {
            return null;
        }

        if (is_numeric($row["data_nascimento"])) {
            $data_nascimento = Date::excelToDateTimeObject($row["data_nascimento"])->format("Y-m-d");
        } else {
