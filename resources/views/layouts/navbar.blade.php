<nav class="navbar {{ session('tema') == "escuro" ? "bg-dark" : "bg-primary" }} bg-gradient border-5 border-bottom border-black shadow mb-1">
    <div class="container-fluid">
        <div class="d-flex align-items-center">
            ...
            <div class="navbar-brand fs-5 fs-lg-3 fw-bold ms-2 ms-md-5 text-center text-md-start">
                <a href="{{ route("home") }}" class="link-offset-2 link-underline link-underline-opacity-0 d-flex gap-1 flex-wrap align-items-center text-black">
                    <svg class="{{ session('tema') == "escuro" ? "text-white" : "text-dark" }} me-1" id="logo_efeito" xmlns="{{ asset("http://www.w3.org/2000/svg") }}" width="50" height="50" fill="currentColor" class="bi bi-card-image" viewBox="0 0 16 16">
                        <path d="M6.002 5.5a1.5 1.5 0 1 1-3 0 1.5 1.5 0 0 1 3 0"/>
                        <path d="M1.5 2A1.5 1.5 0 0 0 0 3.5v9A1.5 1.5 0 0 0 1.5 14h13a1.5 1.5 0 0 0 1.5-1.5v-9A1.5 1.5 0 0 0 14.5 2zm13 1a.5.5 0 0 1 .5.5v6l-3.775-1.947a.5.5 0 0 0-.577.093l-3.71 3.71-2.66-1.772a.5.5 0 0 0-.63.062L1.002 12v.54L1 12.5v-9a.5.5 0 0 1 .5-.5z"/>
                    </svg>
                    {{ config("app.name") }}:
                    <span class="text-white ms-auto">{{ strtoupper($pagina) }}</span>
                </a>
            </div>
        </div>
        ...
        @elseif($pagina === "Cadastro")
            <div class="d-flex flex-column flex-md-row align-items-start align-items-md-center me-5">
                <a href="{{ route("login") }}" style="--bs-icon-link-transform: translate3d(-.250rem, 0, 0);" class="btn btn-lg {{ session('tema') == "escuro" ? "btn-secondary" : "btn-info" }} border icon-link icon-link-hover focus-ring focus-ring-light my-1">
        ...
        @elseif($pagina === "Atualização" || $pagina === "Redefinir Senha")
            <div class="d-flex flex-column flex-md-row align-items-start align-items-md-center me-5">
                <a href="{{ route("home") }}" style="--bs-icon-link-transform: translate3d(-.250rem, 0, 0);" class="btn btn-lg {{ session('tema') == "escuro" ? "btn-secondary" : "btn-info" }} border icon-link icon-link-hover focus-ring focus-ring-light my-1">
                    <svg xmlns="{{ asset("http://www.w3.org/2000/svg") }}" width="16" height="16" fill="currentColor" class="bi bi-arrow-90deg-left me-1" viewBox="0 0 16 16">
                        <path fill-rule="evenodd" d="M1.146 4.854a.5.5 0 0 1 0-.708l4-4a.5.5 0 1 1 .708.708L2.707 4H12.5A2.5 2.5 0 0 1 15 6.5v8a.5.5 0 0 1-1 0v-8A1.5 1.5 0 0 0 12.5 5H2.707l3.147 3.146a.5.5 0 1 1-.708.708z"/>
                    </svg>                    
                    Cancelar
        ...
        @auth
            <div class="btn-group d-flex flex-row align-items-start align-items-md-center me-5 mb-1">
                ...
                <button style="height: 54px;" type="button" class="btn {{ session('tema') == 'escuro' ? 'btn-secondary' : 'btn-info' }} dropdown-toggle dropdown-toggle-split border focus-ring focus-ring-light" data-bs-toggle="dropdown" aria-expanded="false">                    
