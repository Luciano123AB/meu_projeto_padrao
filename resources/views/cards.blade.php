@extends("layouts.main_layout")

@section("content")
    <nav class="navbar bg-primary bg-gradient border-5 border-bottom border-black shadow mb-5">
        <div class="container-fluid">
            <div class="navbar-brand fs-3 fw-bold ms-5">
                <a href="{{ route("home") }}" class="link-offset-2 link-underline link-underline-opacity-0">
                    <svg class="me-1 text-dark" id="logo_efeito" xmlns="{{ asset("http://www.w3.org/2000/svg") }}" width="75" height="75" fill="currentColor" class="bi bi-card-image" viewBox="0 0 16 16"><path d="M6.002 5.5a1.5 1.5 0 1 1-3 0 1.5 1.5 0 0 1 3 0"/><path d="M1.5 2A1.5 1.5 0 0 0 0 3.5v9A1.5 1.5 0 0 0 1.5 14h13a1.5 1.5 0 0 0 1.5-1.5v-9A1.5 1.5 0 0 0 14.5 2zm13 1a.5.5 0 0 1 .5.5v6l-3.775-1.947a.5.5 0 0 0-.577.093l-3.71 3.71-2.66-1.772a.5.5 0 0 0-.63.062L1.002 12v.54L1 12.5v-9a.5.5 0 0 1 .5-.5z"/></svg>
                </a>

                Meu Projeto <span class="text-white">Padrão</span>
            </div>

            <div class="me-5">
                <a href="{{ route("home") }}" style="--bs-icon-link-transform: translate3d(0, -.125rem, 0);" id="cadastrar" class="btn btn-lg btn-info border icon-link icon-link-hover focus-ring focus-ring-light my-1" type="button"><svg xmlns="{{ asset("http://www.w3.org/2000/svg") }}" width="16" height="16" fill="currentColor" class="bi bi-arrow-90deg-left" viewBox="0 0 16 16"><path fill-rule="evenodd" d="M1.146 4.854a.5.5 0 0 1 0-.708l4-4a.5.5 0 1 1 .708.708L2.707 4H12.5A2.5 2.5 0 0 1 15 6.5v8a.5.5 0 0 1-1 0v-8A1.5 1.5 0 0 0 12.5 5H2.707l3.147 3.146a.5.5 0 1 1-.708.708z"/></svg>Voltar ao Home</a>
            </div>
        </div>
    </nav>

    <div class="p-3">
        <div style="height: 750px;" class="container card d-flex border border-black shadow my-5 overflow-y-scroll">
            <div class="row row-cols-2 mx-1 mt-4">
                @forelse($usuarios as $usuario)
                    <div id="cards_efeito" class="col d-flex align-items-stretch">
                        <div class="card mb-4 border-2 border-black w-100 shadow">
                            <div class="row g-0 p-1">
                                <div class="col-md-4 bg-body-secondary p-1 border-end d-flex align-items-center">
                                    <img style="height: 190px;" class="img-fluid rounded-pill border border-black w-100" src="data:image/png;base64,{{ $usuario->foto }}">
                                </div>
                        
                                <div class="col-md-8">
                                    <div class="card-body">
                                        <h5 class="card-title text-center">Informações:</h5>
                                    </div>
                
                                    <ul class="list-group list-group-flush">
                                        <li class="list-group-item">Nome: {{ $usuario->nome_completo }}</li>
                                        <li class="list-group-item">Login: {{ $usuario->usuario }}</li>
                                        <li class="list-group-item">Email: {{ $usuario->email }}</li>
                                        <li class="list-group-item">Senha: {{ "*****" }}</li>
                                        <li class="list-group-item">CPF: {{ "***.***.***-**" }}</li>
                                        <li class="list-group-item">Data de Nascimento: {{ date("d/m/Y", strtotime($usuario->data_nascimento)) }}</li>
                                        <li class="list-group-item">Celular: {{ $usuario->celular }}</li>
                                        <li class="list-group-item">Gênero: {{ $usuario->genero }}</li>
                                        @if($usuario->permissao == 1)
                                            <li class="list-group-item">Permissão: <button class="btn btn-success btn-sm" disabled>SIM</button></li>
                                        @else
                                            <li class="list-group-item">Permissão: <button class="btn btn-danger btn-sm" disabled>NÃO</button></li>
                                        @endif
                                    </ul>
                                </div>
                            </div>
                        </div>
                    </div>
                @empty
                    <div id="cards_efeito" class="col d-flex align-items-stretch">
                        <div class="card mb-4 border-2 border-black w-100 shadow">
                            <div class="row g-0 p-1">
                                <div class="col-md-4 bg-body-secondary p-1 border-end d-flex align-items-center">
                                    <img style="height: 190px;" class="img-fluid rounded-pill border border-black w-100" src="data:image/png;base64,">
                                </div>
                        
                                <div class="col-md-8">
                                    <div class="card-body">
                                        <h5 class="card-title text-center">Informações:</h5>
                                    </div>
                
                                    <ul class="list-group list-group-flush">
                                        <li class="list-group-item">Nome: {{ "..." }}</li>
                                        <li class="list-group-item">Login: {{ "..." }}</li>
                                        <li class="list-group-item">Email: {{ "..." }}</li>
                                        <li class="list-group-item">Senha: {{ "..." }}</li>
                                        <li class="list-group-item">CPF: {{ "..." }}</li>
                                        <li class="list-group-item">Data de Nascimento: {{ "..." }}</li>
                                        <li class="list-group-item">Celular: {{ "..." }}</li>
                                        <li class="list-group-item">Gênero: {{ "..." }}</li>
                                        <li class="list-group-item">Permissão: <button class="btn btn-secondary btn-sm" disabled>NULL</button></li>
                                    </ul>
                                </div>
                            </div>
                        </div>
                    </div>
                @endforelse
            </div>
        </div>
    </div>
@endsection