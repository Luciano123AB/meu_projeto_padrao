<tr>
    <td class="text-center border-end">
        @if(auth()->user()->usuario == "Administrador" || auth()->user()->permissao == 1)
            <a href="{{ route("update", ["id" => Crypt::encrypt($usuario->id)]) }}" class="btn btn-sm {{ Cache::get('tema') === 'escuro' ? "btn-secondary" : "btn-primary" }} m-1" data-bs-toggle="tooltip" title="Editar usuário">
                <i class="bi bi-pencil-square"></i>
            </a>
                
            <a href="{{ route("deletar", ["id" => Crypt::encrypt($usuario->id)]) }}" class="btn btn-sm {{ Cache::get('tema') === 'escuro' ? "btn-dark" : "btn-danger" }}" data-bs-toggle="tooltip" title="Excluir usuário">
